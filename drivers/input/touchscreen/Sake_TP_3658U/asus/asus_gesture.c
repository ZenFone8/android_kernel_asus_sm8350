...
void asus_gesture_report(struct fts_ts_data *ts_data, int gesture_id)
{
	...
	FTS_DEBUG("gesture_id:0x%x, fp_report_type:%d", gesture_id,
	...
	case GESTURE_O:
		...
		if ((ts_data->aod_enable == ENABLE) &&
		    (ts_data->fp_report_type == 0)) { // AOD only
	...
	if (gesture != -1) {
		if (proxy_skip != true) {
			...
			if (proxy_status) {
				FTS_INFO(
					"Gesture Code %d Not FOD gesture and proximity on",
					gesture);
				return;
			}
		}
...
int set_gesture_register(struct fts_ts_data *ts_data)
{
	...
	FTS_INFO("reg D1:0x%2X D2:0x%2X D5:0x%2X D6:0x%2X D7:0x%2X", reg_D1,
		 reg_D2, reg_D5, reg_D6, reg_D7);
	return ret;
}
...
static ssize_t asus_gesture_proc_type_write(struct file *filp, const char *buff,
					    size_t len, loff_t *off)
{
	...
	if (fts_data->gesture_mode_enable == ENABLE) {
		...
		FTS_INFO("gesture_mode_enable type = %#x !",
			 fts_data->gesture_type);
	} else {
		fts_data->gesture_mode_enable = DISABLE;
		fts_data->music_control = DISABLE;
...
static ssize_t asus_gesture_proc_swipeup_read(struct file *file,
					      char __user *buf, size_t count,
					      loff_t *ppos)
{
	int len = 0;
...
static ssize_t asus_gesture_proc_swipeup_write(struct file *filp,
					       const char *buff, size_t len,
					       loff_t *off)
{
	char messages[256];
