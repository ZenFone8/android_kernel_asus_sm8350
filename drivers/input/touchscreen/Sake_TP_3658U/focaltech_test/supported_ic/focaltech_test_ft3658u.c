...
*****************************************************************************/
static int short_test_ch_to_all(struct fts_test *tdata, int *adc, u8 *ab_ch,
				bool *result)
{
	int ret = 0;
	int i = 0;
...
static int short_test_ch_to_gnd(struct fts_test *tdata, int *adc, u8 *ab_ch,
				bool *result)
{
	int ret = 0;
	int i = 0;
...
static int short_test_ch_to_ch(struct fts_test *tdata, int *adc, u8 *ab_ch,
			       bool *result)
{
	int ret = 0;
	int i = 0;
	...
	for (i = 0; i < ab_ch_num; i++) {
		for (j = i + 1; j < ab_ch_num; j++) {
			...
			if (denominator == 0) {
				...
				numerator = (code - offset + 395) * 112;
				short_res[adc_cnt] =
					fts_abs(numerator / denominator - 3);
			}
...
test_err:
	...
	/* save test data */
	fts_test_save_data("Rawdata Test", CODE_M_RAWDATA_TEST, rawdata, 0,
...
test_err:
	...
	fts_test_save_data("Rawdata Uniformity Test",
			   ...
			   tdata->node.node_num * 2, false, false,
			   *test_result);
...
static int ft5652_scap_cb_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (thr->basic.scap_cb_hi_check && hov_high) {
		scb_tmp = scap_cb + scb_cnt;
		/* 1:waterproof 0:non-waterproof */
		ret = get_cb_mc_sc(HIGH_SENSITIVITY, byte_num, scb_tmp,
				   DATA_TWO_BYTE);
		if (ret < 0) {
			FTS_TEST_SAVE_ERR("read sc_cb fail,ret=%d\n", ret);
			goto restore_reg;
	...
	if (thr->basic.scap_cb_hov_check && hov_high) {
		...
		for (i = 0; i < 4; i++) {
			if ((scb_tmp[i] < thr->scap_cb_hov_min[i]) ||
				...
				FTS_TEST_SAVE_ERR(
					"test fail,hov%d=%5d,range=(%5d,%5d)\n",
					i + 1, scb_tmp[i],
...
static int ft5652_scap_rawdata_test(struct fts_test *tdata, bool *test_result)
{
	...
	if (!thr->scap_rawdata_on_min || !thr->scap_rawdata_on_max ||
		...
		FTS_TEST_SAVE_ERR(
			"scap_rawdata_on/off/hi/hov_min/max is null\n");
		ret = -EINVAL;
		goto test_err;
	}
...
test_err:
	...
	fts_test_save_data("Noise Test", CODE_M_NOISE_TEST, noise, 0, false,
			   false, *test_result);
...
static int param_init_ft5652(void)
{
	...
	get_value_basic("SCapNoiseTest_SetHigh_ON",
			&thr->basic.scap_noise_hi_check);
	get_value_basic("SCapNoiseTest_SetHigh_ON",
			&thr->basic.scap_noise_hi_check);
	get_value_basic("SCapNoiseTest_SetWaterproof_ON",
			&thr->basic.scap_noise_wp_on_check);
	get_value_basic("SCapNoiseTest_SetWaterproof_OFF",
			&thr->basic.scap_noise_wp_off_check);
	get_value_basic("NullNoiseTest_Coef", &thr->basic.null_scan_value);
	get_value_basic("SCapNoiseTest_ON_Threshold",
			&thr->basic.scap_noise_on_max);
	get_value_basic("SCapNoiseTest_OFF_Threshold",
			&thr->basic.scap_noise_off_max);
	get_value_basic("SCapNoiseTest_High_Threshold",
			&thr->basic.scap_noise_hi_max);
